...
// =======================
  ...
  function individual()
  {
	...
	if(plotter && plotter.probNorm){probNorm=plotter.probNorm;}
	else{if(item.likelihood && item.likelihood.probNorm){probNorm=item.likelihood.probNorm};};
	 ...
	 && item.likelihood.prob && plotOptions.showDistribution)
	{
	 ...
	 x[i+1]=x[i];
	 y[0]=0;
	 if(lhood.probNorm && probNorm)
	 {
	  for(i=0;i<lhood.prob.length;i++){y[i+1]=lhood.prob[i]*lhood.probNorm/probNorm;};
	 }
	 else
	 {
	...
	if(model && model.element[ind] && model.element[ind].kde_ensembles && plotOptions.showEnsembles && plotOptions.showPosterior)
	{
	 ...
	 for(j=0;(j<model.element[ind].kde_ensembles.length)&&(j<plotOptions.showEnsembles);j++)
	 {
	  ...
	  x[i+1]=x[i];
	  y[0]=0;
	  for(i=0;i<kde_e.prob.length;i++){y[i+1]=kde_e.prob[i]*kde_e.probNorm/probNorm;};
	  y[i+1]=0;
	...
	if(model && model.element[ind] && model.element[ind].kde_mean && (Number(plotOptions.showEnsembles)>-1) && plotOptions.showPosterior)
	{
	 ...
	 x[i+1]=x[i];
	 y[0]=0;
	 for(i=0;i<kde_e.prob_sigma.length;i++){y[i+1]=kde_e.prob_sigma[i][0]*kde_e.probNorm/probNorm;};
	 y[i+1]=0;
	 ...
	 {
	  y.push((kde_e.prob_sigma[i][0]+kde_e.prob_sigma[i][1])*kde_e.probNorm/probNorm);
	 };
	 for(i=kde_e.prob_sigma.length-1;i>=0;i--)
	 {
	  sig=(kde_e.prob_sigma[i][0]-kde_e.prob_sigma[i][1])*kde_e.probNorm/probNorm
	  if(sig>0)
	  {
	   y.push(sig);
	  }
	 ...
	 if(plotter)
	 {
	  ...
	  if(plotOptions.showRanges)
	  {
	   mainViewport.mapRectangle(plotOptions.plotPosX,plotOptions.plotPosY+plotOptions.rangeMargin*plotOptions.plotHeight-miny*plotOptions.distHeight*plotOptions.plotHeight/(maxy-miny),
	    plotOptions.plotWidth,plotOptions.distHeight*plotOptions.plotHeight/(maxy-miny),0,0,plotOptions.plotWidth,probNorm);
	  }
	  else
	  {
	  ...
	  mainViewport.axis("y",0,probNorm,0,0);
	 };
	};
	 ...
	 if(model && model.group && model.group.length)
	 {
	  ...
	  for(m=0;m<model.element.length;m++)
	  {
	   if(plotOptions.showEnsembles && model.element[m] && typeof(model.element[m].age_depth_ensembles)!='undefined')
	   {
	    ...
	    {
	     if(model.element[m].age_depth_z.length>1)
	     {
