...
if __name__ == '__main__':
	...
	for dists in [dists0, dists1, dists2, dists3]:
		for i, s in enumerate(samples):
			...
			t_max = max(t_max, r1)
	if T_TO is not None:
		t_min = max(t_min, 1950 - T_TO)
	if T_FROM is not None:
		t_max = min(t_max, 1950 - T_FROM)
	...
	G3, pos3, groups, gap = get_graph(reduce_earlier_than(earlier_than2), samples)
	plot_graph(	"Stage 3 - Chronological Clustering",
		 	G3, pos3, 'k', 'k', dists0, dists2, samples, outliers, phases2, clusters, DIRECTORY, '08-stage3a', t_min, t_max),
	'''
