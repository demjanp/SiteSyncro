...
function checkForOldCode()
{
 ...
 parent.command=parent.command.replace(/[‘’‛′´`]/g,"'");
 parent.command=parent.command.replace(/[“”‟″˝]/g,'"');
 // check for commas in z,long and lat
 parent.command=parent.command.replace(/(z\s*=\s*[0-9]*)(,)([0-9]*\s*;)/g,'$1.$3');
 parent.command=parent.command.replace(/(longitude\s*=\s*[0-9]*)(,)([0-9]*\s*;)/g,'$1.$3');
 parent.command=parent.command.replace(/(latitude\s*=\s*[0-9]*)(,)([0-9]*\s*;)/g,'$1.$3');
 // check for old Delta_R values in code
 if((parent.command.indexOf("Marine20")!=-1)&&(parent.command.indexOf("Delta_R")!=-1)&&
  (parent.command.indexOf("updated for Marine20")==-1))
 {
  if(window.confirm("Check that Delta_R values have been\nupdated for Marine20!"))
  {
   parent.command="// Delta_R values updated for Marine20\n"+parent.command;
   writeModel(true);
  };
 };
};
